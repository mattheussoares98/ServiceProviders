...
- [ ] Complete
- **Layer:** UI validation.
- **Actions/files:** Area form inside location workflows; back/cancel/reload, move/delete dependency and stale-parent cases. Inspect `lib/features/locations/presentation/` and relevant routed/shared UI. Retain selected proposed real-app journeys for the later device-validation stage; execute these with recorded manual steps on the agreed platforms when that stage resumes.
...
- **Evidence:** Real-app validation remains outstanding and deferred. Historical automated form results and known defects are retained in the [execution report](../../testing/application_validation_progress.md), but creating further widget tests is excluded from this plan.
