...
- [ ] Complete
- **Layer:** UI validation.
- **Actions/files:** Users/permissions editors and invitations with independent actors; verify hidden controls plus direct denied requests. Inspect `lib/features/users/presentation/` and relevant routed/shared UI. Retain selected proposed real-app journeys for the later device-validation stage; execute these with recorded manual steps on the agreed platforms when that stage resumes.
