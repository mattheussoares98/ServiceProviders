...
- [ ] Complete
- **Layer:** UI validation.
- **Actions/files:** Multi-device offline/online and interruption journeys; show queued/failed/committed distinction and usable retry with server evidence. Inspect `lib/features/sync/presentation/` and relevant routed/shared UI. Retain selected proposed real-app journeys for the later device-validation stage; execute these with recorded manual steps on the agreed platforms when that stage resumes.
