...
- [ ] Complete
- **Layer:** UI validation.
- **Actions/files:** Settings cards, accessible toggles and danger-zone confirmation; full save/reload persistence, independent pending-work verification. Inspect `lib/features/configurations/presentation/` and relevant routed/shared UI. Retain selected proposed real-app journeys for the later device-validation stage; execute these with recorded manual steps on the agreed platforms when that stage resumes.
