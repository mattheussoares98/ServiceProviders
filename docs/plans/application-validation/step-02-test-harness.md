...
- **Actions/files:** Inventory existing unit/domain/state/contract cases and relevant coverage using `test/` and `testing/mocks/`. Add a case-to-feature manifest and retain baseline artifacts under this plan's run record. Keep live opt-in absent. Document pre-existing failures without weakening assertions.
