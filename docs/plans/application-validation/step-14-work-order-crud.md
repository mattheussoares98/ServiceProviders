...
- [ ] Complete
- **Layer:** UI validation.
- **Actions/files:** Internal/provider forms, list/detail/deleted filter/restore journeys with PERSIST; follow lifecycle transitions in Step 15. Inspect `lib/features/work_orders/presentation/` and relevant routed/shared UI. Retain selected proposed real-app journeys for the later device-validation stage; execute these with recorded manual steps on the agreed platforms when that stage resumes.
