...
- [ ] Complete
- **Layer:** UI validation.
- **Actions/files:** Sectors CRUD and selection during pause/completion; full PERSIST on supported mutations. Inspect `lib/features/sectors/presentation/` and relevant routed/shared UI. Retain selected proposed real-app journeys for the later device-validation stage; execute these with recorded manual steps on the agreed platforms when that stage resumes.
