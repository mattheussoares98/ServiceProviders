...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockWorkOrdersCubit.state).thenReturn(const WorkOrdersState.initial());
    when(() => mockWorkOrdersCubit.stream).thenAnswer((_) => const Stream.empty());

    when(() => mockPauseWorkflowCubit.state).thenReturn(const PauseWorkflowState.initial());
    when(() => mockPauseWorkflowCubit.stream).thenAnswer((_) => const Stream.empty());
  ...
  group('WorkOrderBottomActions', () {
    testWidgets('renders Iniciar trabalho when status is open', (tester) async {
      final workOrder = EntityFactory.makeWorkOrderEntity().copyWith(
    ...
    testWidgets('renders Retomar trabalho when status is onHold', (tester) async {
      final workOrder = EntityFactory.makeWorkOrderEntity().copyWith(
    ...
    testWidgets('renders Pausar and Solicitar conclusão when inProgress and user has no manage permission', (tester) async {
      final workOrder = EntityFactory.makeWorkOrderEntity().copyWith(
    ...
    testWidgets('renders Concluir when inProgress and user has manage permission', (tester) async {
