...
void main() {
  ...
  group('getPauseRequestsByWorkOrderIds', () {
    test(
      'fetches remotely and saves to local data source when online',
      () async {
        when(() => mockInternetClient.isConnected).thenReturn(true);
        when(
        ...
        verify(
          () => mockLocalDataSource.savePauseRequests([tRequestModel]),
        ).called(1);
      },
    );
    ...
    test('fetches from local data source when offline', () async {
      ...
      expect(result.data?.first.id, tRequestEntity.id);
      verify(
        () => mockLocalDataSource.getPauseRequestsByWorkOrderIds([
          tRequestEntity.workOrderId,
        ]),
      ).called(1);
    });
