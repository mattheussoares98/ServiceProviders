...
void main() {
  ...
  group('AssetsRemoteDataSourceImpl', () {
    test('should return SuccessState<List<AssetResponseModel>> on 200 (getAssets)', () async {
      // Arrange
      final fakeResponse = {
      ...
      // Assert
      expect(result, isA<SuccessState<List<AssetResponseModel>>>());
    ...
    test('should return SuccessState<AssetResponseModel> on 200 (getAssetById)', () async {
      // Arrange
      final fakeResponse = {
        'data': tAssetModel.toJson(),
        'message': 'Success',
      ...
      // Assert
      expect(result, isA<SuccessState<AssetResponseModel>>());
      expect(result.data!.id, tAssetModel.id);
      verify(() => mockHttpClient.get<dynamic>(ApiEndpoints.assetById(tId))).called(1);
    });
    ...
    test('should return SuccessState<AssetResponseModel> on update', () async {
      ...
      when(() => mockHttpClient.put<dynamic>(
        ...
        (_) async => Response(
          requestOptions: RequestOptions(path: ApiEndpoints.assetById(tAssetRequest.id)),
          data: fakeResponse,
          statusCode: 200,
        ),
      );
    ...
    test('should return SuccessState<void> on delete', () async {
      ...
      when(() => mockHttpClient.delete<dynamic>(
        ...
        (_) async => Response(
          requestOptions: RequestOptions(path: ApiEndpoints.assetById(tAssetModel.id)),
          data: fakeResponse,
          statusCode: 200,
        ),
      );
