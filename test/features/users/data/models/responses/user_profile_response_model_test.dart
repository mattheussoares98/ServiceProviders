...
void main() {
  ...
  group('UserProfileResponseModel', () {
    ...
    test('should serialize to JSON with correct flat format overrides on toJson', () {
      final model = UserProfileResponseModel.fromEntity(tEntity).copyWith(
    ...
    test('should deserialize generic actions and scopes correctly on fromJson', () {
      final json = {
        'id': 'user-123',
      ...
      expect(result.permissions[ResourceType.attachments]?[PermissionAction.create], true);
      expect(result.permissions[ResourceType.attachments]?[PermissionAction.delete], false);
      expect(result.permissions[ResourceType.attachments]?[PermissionAction.update], isNull);

      // Scoped work orders permission override check
      expect(result.workOrdersPermissionOverrides.readScope, WorkOrderReadScope.assigned);
      expect(result.workOrdersPermissionOverrides.updateScope, WorkOrderUpdateScope.own);
